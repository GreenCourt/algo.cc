...
Float convex_diameter(const Polygon &convex) {
#ifdef _GLIBCXX_ASSERTIONS
  assert(is_convex(convex));
#endif
  int n = ssize(convex);
  ...
  for(int i=0; i<n; ++i) if(abs(c = ccw(convex[i], convex[(i+1)%n], convex[(i+2)%n])) == 1) break;
  assert(abs(c)==1);
  auto lt=[](const Point& l, const Point &r) {
    return sgn(real(l-r)) ? real(l-r) < 0 : sgn(imag(l-r)) < 0; };
  int i=0, j=0;
  for(int k=0; k<n; ++k) {
    if(!lt(convex[i], convex[k])) i=k;
    if(lt(convex[j], convex[k])) j=k;
  }
  double d = 0; int start_i = i, start_j = j;
  while(i != start_j || j != start_i) {
    d = max(d, abs(convex[i] - convex[j]));
    if(c*cross(convex[(i+1)%n] - convex[i], convex[(j+1)%n] - convex[j]) < 0) i = (i+1) % n;
    else j = (j+1) % n;
  }
  return d;
}
